...
export default function PaperDetail() {
  ...
  // ↔️ Resizable Chat Panel State
  const [chatWidth, setChatWidth] = useState(450); // Default width in px
  ...
  return (
    <div className="h-screen w-screen bg-[#FDFDFF] text-slate-800 font-interface relative overflow-hidden flex flex-col">
      {/* 🔮 AMBIENT BACKDROP */}
      ...
      <nav className="w-full h-16 shrink-0 bg-white/80 backdrop-blur-md border-b border-purple-100 relative z-50">
        <div className="max-w-full px-6 h-full flex justify-between items-center">
          <div
            className="flex items-center gap-2.5 cursor-pointer group"
            onClick={handleHomeNavigate}
          >
            <div className="w-9 h-9 bg-gradient-to-tr from-[var(--color-brand-primary)] to-[var(--color-brand-accent)] rounded-xl flex items-center justify-center shadow-md shadow-purple-600/10 transition-transform duration-300 group-hover:scale-105">
          ...
          {isAuthenticated ? (
            ...
            >
              ...
              <div className="w-8 h-8 bg-gradient-to-tr from-[var(--color-brand-primary)] to-[var(--color-brand-accent)] rounded-full flex items-center justify-center text-xs font-black text-white shadow-md shadow-purple-600/20 ring-2 ring-white">
                {userInitials}
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <button
                ...
                className="text-xs font-bold text-slate-600 hover:text-[var(--color-brand-primary)] px-3.5 py-2 transition-colors cursor-pointer"
              >
                Sign In
              </button>
              <button
                onClick={() => navigate("/signup")}
                className="text-xs font-bold text-white bg-gradient-to-r from-[var(--color-brand-primary)] to-[var(--color-brand-accent)] hover:opacity-90 px-4 py-2 rounded-xl transition-all shadow-sm cursor-pointer whitespace-nowrap"
              >
                Create Account
              </button>
            </div>
      ...
      <div className="flex-1 flex overflow-hidden relative z-10 p-4 md:p-6 gap-4">
        {/* LEFT COLUMN: Independent Scrollable Details Panel */}
        <div className="flex-1 h-full overflow-y-auto pr-2 space-y-6">
          {/* Nav buttons */}
          <div className="flex items-center gap-3">
            ...
            <motion.button
              ...
              onClick={handleHomeNavigate}
              className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-[var(--color-brand-primary)] bg-white border border-purple-100 px-4 py-2 rounded-xl shadow-sm transition-all cursor-pointer"
            >
              ← Home
            </motion.button>
          </div>
          ...
          >
            ...
            {externalLink && (
              <a
                ...
                className="inline-flex items-center gap-1.5 bg-gradient-to-r from-[var(--color-brand-primary)] to-[var(--color-brand-accent)] text-white text-xs font-bold px-5 py-2.5 rounded-xl hover:opacity-90 active:scale-95 transition-all shadow-md shadow-purple-600/10"
              >
                Open Original Paper ↗
              </a>
            )}
        ...
        {/* RIGHT COLUMN: Full Height Chat Panel Container */}
        <div
          style={{ width: `${chatWidth}px` }}
          className="h-full shrink-0 flex flex-col transition-none"
        >
          {chatStarted ? (
            <ChatPanel paperId={effectivePaperId} paperTitle={paper.title} />
          ) : (
            <div className="bg-white border border-purple-100 rounded-[2rem] p-7 shadow-sm flex flex-col items-center text-center gap-3 h-full justify-center">
